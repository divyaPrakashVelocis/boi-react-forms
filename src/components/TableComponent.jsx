...
function TableComponent() {
    ...
    const deriveRightsFromRoles = (roles) => {
        ...
        const isFormAdmin = hasRole(roles, "FORM_ADMIN_HINDI_BRANCH_P2_UPDATE");
        const isZoneAdmin = hasRole(roles, "ZONE_ADMIN_HINDI_BRANCH_P2_UPDATE");
        const isUser = hasRole(roles, "USERROLE");
        const hasAllRights = isUser || isHoAdmin || isFormAdmin || isZoneAdmin;
        return {
            canAdd: isUser && !isHoAdmin && !isFormAdmin && !isZoneAdmin,
            canEdit: hasAllRights,
            canDelete: !isUser || isHoAdmin || !isFormAdmin,
