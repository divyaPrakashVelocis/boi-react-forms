...
const FormButtons = ({
        ...
        try {
            ...
            if (response.success) {
                ...
                if (err?.response?.status === 409) {
                    ...
                    navigate("/hindi-qpr-branch-2-updated");
                }
            }
        } catch (error) {
    ...
    const viewPDF = () => {
        ...
        const formType = "branch2";
        let URL = "";
        if (!!port1) {
            // http://localhost:8080/pdf?type=branch2&id=1
            URL = `${window.location.protocol}//${window.location.hostname}:${window.location.port}/pdf?type=${formType}&id=${iidd}`;
        } else {
            URL = `${window.location.protocol}//${window.location.hostname}/pdf?type=${formType}&id=${iidd}`;
        }
    ...
    return (
        <div className="button-group mt-3 d-flex justify-content-center">
            <button
                ...
                type="button"
                onClick={() => navigate("/hindi-qpr-branch-2-updated")}
            >
                Cancel
            </button>
            {(mode === "add" ||
